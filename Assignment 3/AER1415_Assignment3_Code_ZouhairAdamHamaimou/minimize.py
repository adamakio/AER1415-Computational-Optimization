...
#
## PBN edits
## (1) The user provides a single function that returns the objective function
#      value and its gradient
#  (2) Create log of history of l2 norm of gradients 
#  (3) Record total number of function evaluations after successful line search
#
#
...
def minimize(X, f, args, maxnumlinesearch=None, maxnumfuneval=None, red=1.0, verbose=True):
    ...
    Args:
        X: initial guess
        f: function to be minimized. Must return a tuple (f, df) where f is the
           function value and df is the gradient.
        args: additional arguments to be passed to f
        maxnumlinesearch: maximum number of line searches
        maxnumfuneval: maximum number of function evaluations
        red: initial step size for line search
        verbose: if True, print progress information
    Returns:
        X: the point where the function is minimized
        fX: history of function values
        gradfX: history of l2 norm of gradients
        nf: history of number of function evaluations
    """
    ...
    if maxnumlinesearch == None:
        if maxnumfuneval == None:
            ...
            length = -maxnumfuneval #PBN - use negative sign to make sure the function eval counter
                                    #      works correctly
    else:
        if maxnumfuneval != None:
    ...
    while i < abs(length):                                 # while not finished
            ...
            while (not success) and (M > 0):
                try:
                    ...
                    if isnan(f3) or isinf(f3) or any(isnan(df3)+isinf(df3)):
                        print("error - encountered nan or inf in function value or gradient")
                        return
                    success = 1
        ...
        if abs(d3) < -SIG*d0 and f3 < f0+x3*RHO*d0:  # if line search succeeded
            ...
            gradfX.append(norm(df3)); nf.append(i); # update variables
